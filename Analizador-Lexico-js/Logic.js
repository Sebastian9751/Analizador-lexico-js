...
.addEventListener('change', leerTxt, false);
...
function validarEstructura(contenido){
     ...
     bloque.forEach(item=>{
      ...
      if(item==inicio && bloque.indexOf(item)==0){
        ...
        if(item!=''&& bloque.indexOf(item)!= bloque.length-2 && item!=final){
      ...
      if(bloque.indexOf(item)== bloque.length-2){
        ...
        if(item==final ){ 
          ...
          console.log(item,"DECLARACION  incorrecta fin del programa no encontrado la linea ", bloque.indexOf(item) );
...
function validarDeclaraciones(bloques){
  ...
  bloques.forEach(item=>{
    item.startsWith('Ent')?
    item.match(/[Ent]\s\w+\s?=?\s?\w+?;?\s?/g)?
    result =[i+1,`<h5 class="var">variable declarada correctamente  ${item}</h5>`]:
    result =[i+1,`<h5 class="error">la declaracion "${item}" es incorrecta</h5>`]
